...
class AnimateLibraryExporter
{
	...
	public function new(swfData:SWFRoot, targetPath:String)
	{
        ...
        // I want the generated classes in the main source code
		// but the uuid keeps changing after every rebuild, this
		// makes it persist as the name of the library instead
		//
		// var uuid = StringTools.generateUUID(20);
        var uuid: String;
        var reg = new EReg("libraries/(.*)\\.zip", "g");
        if (reg.match(targetPath)) {
            uuid = reg.matched(1);
        }
        else {
            uuid = StringTools.generateUUID(20);
            trace(targetPath);
            Log.warn("Using autogenerated uuid " + uuid + "!");
        }
